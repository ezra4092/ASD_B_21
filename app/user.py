...
def login_pengunjung(ll_peminjaman):
    """Menangani proses autentikasi member pengunjung berdasarkan data dari file JSON"""
...
def lihat_status_peminjaman(username_login, ll_peminjaman):
    """Menampilkan riwayat dan status peminjaman buku khusus untuk user yang sedang login"""
...
def user_menu(username_login, ll_peminjaman, is_member):
    """Menampilkan antarmuka menu eksklusif bagi member yang telah berhasil login"""
    while True:
        clear_screen()
        print("=" * 50)
        print("MENU MEMBER")
